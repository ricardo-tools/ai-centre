...
interface ProjectGeneratorMDProps {
  skills: Skill[];
  archetypes: Archetype[];
}

export function ProjectGeneratorMD({ skills, archetypes }: ProjectGeneratorMDProps) {
  const { t } = useLocale();
  ...
  return (
    <div style={{ maxWidth: 640, padding: 16 }}>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--color-text-heading)', margin: '0 0 6px' }}>
        {t('generate.title')}
      </h1>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
          {archetypes.map((a) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-heading)', margin: '0 0 3px' }}>{a.title}</p>
          ...
          >
            ...
            <div>
              <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-heading)', margin: '0 0 3px' }}>{t('generate.custom')}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {skills.map((s) => {
            ...
            return (
              ...
              >
                <Icon size={18} weight={checked ? 'fill' : 'regular'} color={checked ? 'var(--color-primary)' : 'var(--color-text-muted)'} />
                <div style={{ flex: 1 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-heading)' }}>{s.title}</span>
                  {s.isOfficial && (
                    <span style={{ fontSize: 10, padding: '1px 5px', borderRadius: 3, background: 'var(--color-success-muted)', color: 'var(--color-success)', fontWeight: 600, marginLeft: 8 }}>
                      {t('skills.official')}
                    </span>
                  )}
                  <p style={{ fontSize: 11, color: 'var(--color-text-muted)', margin: '2px 0 0', lineHeight: 1.3 }}>{s.description}</p>
                </div>
              </button>
            );
          })}
