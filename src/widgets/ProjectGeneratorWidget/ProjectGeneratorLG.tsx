...
interface ProjectGeneratorLGProps {
  skills: Skill[];
  archetypes: Archetype[];
}

export function ProjectGeneratorLG({ skills, archetypes }: ProjectGeneratorLGProps) {
  const { t } = useLocale();
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
          ...
          >
            ...
            <div>
              <p style={{ fontSize: 15, fontWeight: 600, color: 'var(--color-text-heading)', margin: '0 0 4px' }}>{t('generate.custom')}</p>
              <p style={{ fontSize: 12, color: 'var(--color-text-muted)', margin: 0 }}>{t('generate.customDescription')}</p>
            </div>
          </button>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {s.isOfficial && (
                    <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 3, background: 'var(--color-success-muted)', color: 'var(--color-success)', fontWeight: 600, marginLeft: 8 }}>
                      {t('skills.official')}
                    </span>
                  )}
