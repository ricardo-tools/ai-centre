      ...
      <Section title="Widget Anatomy">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            ...
            <div style={{ fontFamily: 'monospace', fontSize: 13, lineHeight: 2, color: 'var(--color-text-body)' }}>
              ...
              <div style={{ paddingLeft: 20 }}>├─ TopNavSM.tsx <span style={{ color: 'var(--color-text-muted)' }}>← small</span></div>
              <div style={{ paddingLeft: 20 }}>├─ TopNavXS.tsx <span style={{ color: 'var(--color-text-muted)' }}>← mobile</span></div>
              <div style={{ paddingLeft: 20 }}>├─ useTopNav.ts <span style={{ color: 'var(--color-text-muted)' }}>← data hook</span></div>
              <div style={{ paddingLeft: 20 }}>└─ index.ts <span style={{ color: 'var(--color-text-muted)' }}>← barrel export</span></div>
            </div>
          </div>
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
        ...
        <div style={{ marginTop: 16, padding: 16, borderRadius: 8, background: 'var(--color-primary-muted)', border: '1px solid var(--color-border)' }}>
          <p style={{ fontSize: 13, color: 'var(--color-text-body)', margin: 0 }}>
            <strong>Data hook convention:</strong> Every widget data hook MUST support a <code style={{ background: 'var(--color-bg-alt)', padding: '1px 4px', borderRadius: 3 }}>mock?: boolean</code> prop. When <code style={{ background: 'var(--color-bg-alt)', padding: '1px 4px', borderRadius: 3 }}>mock</code> is true, the hook returns hardcoded mock data immediately — no fetch, no loading state. This enables development and testing without a live API.
          </p>
          <CodeBlock language="ts" title="Mock data convention">{`// useTopNav.ts
const MOCK_DATA = { /* ... */ };
...
export function useTopNav(mock = false) {
  ...
  // real fetch...
      ...
      {/* ---- Widget States ---- */}
      <Section title="Widget States">
        <p style={{ fontSize: 14, color: 'var(--color-text-muted)', marginBottom: 20 }}>Every widget data hook must handle three states beyond success. Widgets must never show a blank container, crash silently, or break the grid layout.</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
          {/* Loading */}
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--color-warning)' }} />
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-heading)' }}>Loading</span>
            </div>
            <p style={{ fontSize: 13, color: 'var(--color-text-body)', marginBottom: 12 }}>Show a skeleton placeholder that structurally matches the widget layout. Never show a blank container or a spinner alone.</p>
            <div style={{ padding: 12, borderRadius: 6, background: 'var(--color-bg-alt)' }}>
              <div style={{ height: 12, width: '60%', borderRadius: 4, background: 'var(--color-border)', marginBottom: 8 }} />
              <div style={{ height: 10, width: '90%', borderRadius: 4, background: 'var(--color-border)', marginBottom: 6 }} />
              <div style={{ height: 10, width: '75%', borderRadius: 4, background: 'var(--color-border)' }} />
            </div>
            <p style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 8 }}>Skeleton must match success layout to prevent layout shift.</p>
          </div>
          {/* Error */}
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--color-danger)' }} />
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-heading)' }}>Error</span>
            </div>
            <p style={{ fontSize: 13, color: 'var(--color-text-body)', marginBottom: 12 }}>Inline error message with a retry action. Never crash silently or show a blank state on failure.</p>
            <div style={{ padding: 12, borderRadius: 6, background: 'var(--color-bg-alt)', display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 12, color: 'var(--color-danger)', fontWeight: 600 }}>Failed to load data</span>
              <span style={{ fontSize: 11, color: 'var(--color-secondary)', cursor: 'pointer', textDecoration: 'underline' }}>Retry</span>
            </div>
            <p style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 8 }}>Error state must fit within the widget&apos;s grid cell.</p>
          </div>
          {/* Empty */}
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--color-text-muted)' }} />
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-heading)' }}>Empty</span>
            </div>
            <p style={{ fontSize: 13, color: 'var(--color-text-body)', marginBottom: 12 }}>Contextual message distinguishing &quot;no data exists&quot; vs &quot;no results match filter&quot;.</p>
            <div style={{ padding: 12, borderRadius: 6, background: 'var(--color-bg-alt)', textAlign: 'center' }}>
              <span style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>No invoices found</span>
            </div>
            <p style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 8 }}>Empty state must also fit within the grid cell.</p>
          </div>
        </div>
        {/* Key rule callout */}
        <div style={{ marginTop: 16, padding: 16, borderRadius: 8, background: 'var(--color-primary-muted)', border: '1px solid var(--color-border)' }}>
          <p style={{ fontSize: 13, color: 'var(--color-text-body)', margin: 0 }}>
            <strong>Key rules:</strong> Always wrap fetch calls in <code style={{ background: 'var(--color-bg-alt)', padding: '1px 4px', borderRadius: 3 }}>try/catch</code> and set error state on failure. Loading skeletons must structurally match the success layout to prevent layout shift. Error and empty states must fit within the widget&apos;s assigned grid cell.
          </p>
        </div>
        {/* Code example */}
        <div style={{ marginTop: 16, padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
          <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-muted)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Pattern</p>
          <CodeBlock language="tsx" title="Widget state handling">{`// In the data hook
...
if (!data || data.length === 0) return <EmptyState message="No items found" />;`}</CodeBlock>
        </div>
      </Section>
      ...
      <Section title="Grid System">
        ...
        <div style={{ marginBottom: 20, padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
          ...
          <CodeBlock language="ts" title="Responsive type">{`type Responsive<T> = T | { default: T; sm?: T; md?: T; lg?: T };

// Plain value → same at all breakpoints
columns: 12

// Object → cascades mobile-first: default → sm → md → lg
columns: { default: 1, sm: 2, lg: 12 }`}</CodeBlock>
        </div>
      ...
      <Section title="Screen Config & Slots">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            ...
            <CodeBlock language="ts" title="Screen config">{`class DashboardConfig {
  grid = { columns: 12,
           rows: 'auto',
           rowHeight: 60, gap: 16 };
  theme = 'light';
      ...
      <Section title="Styling Rules">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ padding: 20, borderRadius: 8, border: '2px solid var(--color-success)', background: 'var(--color-surface)' }}>
            ...
            <CodeBlock language="tsx" title="Correct — semantic tokens">{`<div style={{
  ...
  borderRadius: 8,
          ...
          <div style={{ padding: 20, borderRadius: 8, border: '2px solid var(--color-danger)', background: 'var(--color-surface)' }}>
            ...
            <CodeBlock language="tsx" title="Wrong — hardcoded hex">{`<div style={{
  background: '#FFFFFF',
      ...
      <Section title="Localisation">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            ...
            <CodeBlock language="tsx" title="Translation keys">{`// ✅ CORRECT
const { t } = useLocale();
<h2>{t('orders.recentOrders')}</h2>

// ❌ WRONG — hardcoded string
<h2>Recent Orders</h2>`}</CodeBlock>
          </div>
          <div style={{ padding: 20, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
            ...
            <CodeBlock language="tsx" title="Locale-aware formatting">{`// Money.ts
...
<span>{order.total.format(locale)}</span>`}</CodeBlock>
          </div>
        </div>
      ...
      <Section title="Failure Patterns">
        <p style={{ fontSize: 14, color: 'var(--color-text-muted)', marginBottom: 20 }}>Common mistakes and how to fix them. Use this as a review checklist.</p>
        <div style={{ borderRadius: 8, border: '1px solid var(--color-border)', overflow: 'hidden' }}>
          {/* Header */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '10px 16px', background: 'var(--color-bg-alt)', borderBottom: '1px solid var(--color-border)' }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-heading)' }}>Failure</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-heading)' }}>Why It Happens</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-heading)' }}>Fix</span>
          </div>
          {[
            { failure: 'Wrong grid cell position', why: 'Used 0-based indexing for row/column', fix: 'CSS Grid is 1-based — row: 1 is the first row, not row: 0' },
            { failure: 'ACL mapper drift', why: 'API response shape changed but mapper not updated', fix: 'Explicit field mapping in mapper; type-check raw response; never spread raw objects into domain' },
            { failure: 'Data hook never resolves', why: 'Missing error handling in async fetch', fix: 'Always wrap fetch in try/catch; set loading=false in finally block' },
            { failure: 'Widget breaks grid layout', why: 'Error/empty state has different dimensions', fix: 'Error and empty states must fit within the widget\'s assigned grid cell' },
            { failure: 'Hardcoded hex colors', why: 'Developer used #hex instead of semantic tokens', fix: 'Use var(--color-*) exclusively; grep for # in style objects during review' },
            { failure: 'Component fetches data', why: 'Stateless component calls server action or API', fix: 'Components are props-only; data fetching belongs in widget data hooks' },
            { failure: 'Domain object has React import', why: 'Business logic coupled to framework', fix: 'Domain objects must have zero framework dependencies' },
            { failure: 'Screen config uses raw pixels', why: 'Layout defined with hardcoded px instead of grid', fix: 'Screen configs use typed grid declarations, not pixel values' },
          ].map((row, i, arr) => (
            <div key={row.failure} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '10px 16px', borderBottom: i < arr.length - 1 ? '1px solid var(--color-border)' : 'none', background: i % 2 === 0 ? 'var(--color-surface)' : 'var(--color-bg)' }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>{row.fix}</span>
            </div>
          ))}
        </div>
      </Section>
