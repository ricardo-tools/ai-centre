  ...
  return (
    <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            ...
            <div>
              ...
              <ul style={{ fontSize: 13, color: 'var(--color-text-body)', lineHeight: 2, listStyle: 'none', padding: 0 }}>
                ...
                <li>Convert text to outlines/paths as a last resort if embedding isn&apos;t possible</li>
                <li>Line height: 120-150% for body text</li>
                <li>Use rich black (C40 M30 Y30 K100) for large dark areas. For text, use K:100 only to avoid registration issues.</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <Section title="Pre-Flight Checklist">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ...
            'Bleed: 3mm standard, 5mm large posters, 10mm roll-up banners',
      ...
      <Section title="Common Print Format Reference">
        <div style={{ borderRadius: 8, border: '1px solid var(--color-border)', overflow: 'hidden' }}>
          <div style={{ display: 'flex', padding: '8px 16px', background: 'var(--color-bg-alt)', borderBottom: '1px solid var(--color-border)' }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-heading)', width: 140 }}>Bleed (3mm)</span>
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-heading)' }}>Safe (4mm inset)</span>
          </div>
          {[
            { format: 'Business card', trim: '90 × 55 mm', bleed: '96 × 61 mm', safe: '82 × 47 mm' },
            { format: 'Business card (US)', trim: '89 × 51 mm', bleed: '95 × 57 mm', safe: '81 × 43 mm' },
            { format: 'DL flyer', trim: '99 × 210 mm', bleed: '105 × 216 mm', safe: '91 × 202 mm' },
            { format: 'A6 flyer', trim: '105 × 148 mm', bleed: '111 × 154 mm', safe: '97 × 140 mm' },
            { format: 'A5 flyer', trim: '148 × 210 mm', bleed: '154 × 216 mm', safe: '140 × 202 mm' },
            { format: 'A4 flyer/brochure', trim: '210 × 297 mm', bleed: '216 × 303 mm', safe: '202 × 289 mm' },
            { format: 'A3 poster', trim: '297 × 420 mm', bleed: '303 × 426 mm', safe: '289 × 412 mm' },
            { format: 'Square sticker (50mm)', trim: '50 × 50 mm', bleed: '56 × 56 mm', safe: '42 × 42 mm' },
          ].map((row, i) => (
      ...
      <Section title="Failure Patterns">
        <p style={{ fontSize: 14, color: 'var(--color-text-muted)', marginBottom: 20 }}>Common print design failures and how to avoid them.</p>
        <div style={{ borderRadius: 8, border: '1px solid var(--color-border)', overflow: 'hidden' }}>
          <div style={{ display: 'flex', padding: '8px 16px', background: 'var(--color-bg-alt)', borderBottom: '1px solid var(--color-border)' }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-heading)', width: 200 }}>Failure</span>
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-heading)', width: 240 }}>Why It Happens</span>
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-heading)' }}>Fix</span>
          </div>
          {[
            { failure: 'White edge strips after cutting', why: 'Backgrounds don\'t extend to bleed', fix: 'Extend all backgrounds to bleed edge' },
            { failure: 'Text cut off at edges', why: 'Content too close to trim', fix: 'Keep text inside safe zone' },
            { failure: 'Font substitution in PDF', why: 'Fonts not embedded', fix: 'Embed all fonts or convert to outlines' },
            { failure: 'Blurry/pixelated images', why: 'Source below 300 DPI', fix: 'Use high-res source images' },
            { failure: 'Colors look dull/different', why: 'RGB not converted to CMYK', fix: 'Convert to CMYK, soft-proof' },
            { failure: 'Fold misalignment', why: 'Inner panel same width as outer', fix: 'Narrow inner panel by 2\u20133mm' },
            { failure: 'Paper cracks at fold', why: 'No fold scoring on thick stock', fix: 'Score folds on \u2265170gsm' },
            { failure: 'QR code won\'t scan', why: 'Too small or insufficient quiet zone', fix: 'Min 12mm with 4-module quiet zone' },
            { failure: 'Ink smearing/bleeding', why: 'Total ink coverage too high', fix: 'Keep total ink below 300%' },
            { failure: 'Text illegible at print size', why: 'Font too small for medium', fix: 'Min 7pt body, 6pt fine print' },
          ].map((row, i) => (
            <div key={row.failure} style={{ display: 'flex', padding: '8px 16px', borderBottom: i < 9 ? '1px solid var(--color-border)' : 'none', background: i % 2 === 0 ? 'var(--color-surface)' : 'var(--color-bg)' }}>
              <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-danger)', width: 200 }}>{row.failure}</span>
              <span style={{ fontSize: 12, color: 'var(--color-text-muted)', width: 240 }}>{row.why}</span>
