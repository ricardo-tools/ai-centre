...
- **Visibility system:** `showcase_uploads.visibility` and `community_skills.visibility` columns (text, default `'public'`, values: `public`/`private`/`link_only`). `user_quotas.default_visibility` stores user preference. Migration `0018_add_visibility.sql` (IF NOT EXISTS pattern). `fetchAllShowcases` filters by visibility — owner sees all their items, others see only `public` or items shared with them via `resource_shares`. Visibility change API: `PATCH /api/shares/visibility` (owner only).
- **ShareModal:** `src/platform/components/ShareModal/` — rebuilt with visibility selector (radio cards, owner only), people-with-access list with inline permission checkboxes (view/download/reshare), owner row, add-person form, share-links list with copy/revoke. Sub-components: `VisibilitySelector.tsx`, `PermissionCheckbox.tsx`. Props: `resourceType`, `resourceId`, `isOpen`, `onClose`, `isOwner`, `currentVisibility`, `onVisibilityChange`.
- **Upload sharing flow:** `ShowcaseUploadWidget` supports `link_only` visibility mode (third radio card). When private or link_only is selected, shows inline share-with section (email input + permission checkboxes + person list). On publish: creates shares for each person, creates a share link if link_only.
- **Viewer visibility badge:** `ShowcaseViewerWidget` shows colored visibility badge next to title (Globe/LockSimple/LinkSimple icons). Badge updates when visibility changes via ShareModal.
