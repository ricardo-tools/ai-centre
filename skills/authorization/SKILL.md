...
- Use granular permission checks (`hasPermission`) for access decisions. Not: checking roles directly
- Enforce all permissions on the server. Not: client-only permission enforcement
- Assign new users the least-privileged role. Not: admin role by default
- Require auth checks on all endpoints. Not: relying on URL obscurity as access control
- Check permissions at request time from the authoritative source. Not: relying on JWT claims that can't be revoked
- Require admin or workflow approval for role changes. Not: allowing role self-escalation
- Verify resource ownership on every edit and delete operation. Not: skipping ownership checks
