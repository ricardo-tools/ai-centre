...
- Use `next/image` for optimisation, lazy loading, and format conversion. Not: raw `<img>` tags in Next.js
- Use `next/font` for zero-CLS font loading. Not: web fonts via `<link>` or `@import`
- Push `'use client'` down to the smallest interactive component. Not: `'use client'` on page or layout components
- Fetch initial page content in Server Components and pass as props. Not: client-side fetch for initial page content
- Split components >50KB with `dynamic()`. Not: large dependencies in the client bundle without dynamic import
- Set `width` and `height` on every image to reserve space and prevent layout shift. Not: images without dimensions
- Use `next/script` with `lazyOnload` strategy for third-party scripts. Not: scripts in `<head>` blocking render
- Run Lighthouse first, then fix what is actually slow. Not: optimising without measuring
- Always show structure while loading (skeleton screens or progress). Not: blank page to content pop
