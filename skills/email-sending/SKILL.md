...
- Queue email sends asynchronously. Not: sending email synchronously in the request path
- Provide a support email as an alternative contact. Not: no-reply address with no alternative contact
- Require explicit opt-in before sending marketing email. Not: marketing email without opt-in
- Document tracking pixels in the privacy policy. Not: tracking pixels without privacy disclosure
- Store SMTP credentials in environment variables. Not: hardcoding SMTP credentials in code
- Mark hard-bounced addresses as undeliverable and stop sending. Not: sending to hard-bounced addresses
- Use BCC or individual sends for multiple recipients. Not: all recipients in To/CC
