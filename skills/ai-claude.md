...
- ❌ Anthropic SDK imported directly in use cases or UI code → wrap in an infrastructure service
- ❌ Prompts as inline string concatenation in API calls → use structured prompt template functions
- ❌ Trusting raw AI text output as structured data → always validate with a schema (Zod, etc)
- ❌ Single model hardcoded for all tasks → select model per task based on complexity and cost
- ❌ Long generation without streaming → stream any response expected to take >1 second
- ❌ Sending full context when a subset would suffice → send only relevant content, summarise large inputs
- ❌ No error handling for rate limits or overload → handle 429/529 explicitly with backoff and fallback
- ❌ Testing AI features by calling the real API → mock the AI service boundary in unit tests
- ❌ API keys hardcoded or committed → environment variables only, never in code
- ❌ Ignoring token usage and costs → log usage per call, track costs per feature
