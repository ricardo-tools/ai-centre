...
- Use Redis for caching, rate limiting, and ephemeral data only; it is volatile. Not: using Redis as the primary database
- Set a TTL on every cache key so it expires. Not: keys without TTL for cache data
- Use `namespace:entity:id` format for all keys. Not: flat, unnamespaced keys like `"skill123"`
- Store a reference (URL, database ID) instead of large payloads. Not: storing objects >1 MB in Redis
- Track keys explicitly or use TTL for cleanup. Not: using `KEYS *` or `SCAN` in production
- Cache only hot paths where database latency matters. Not: caching everything (premature caching adds complexity without benefit)
- Invalidate or update the cache on every write path that affects cached data. Not: ignoring cache invalidation
