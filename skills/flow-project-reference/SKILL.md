...
Opinion companion for **flow** (core). A living project reference document for context persistence.

**Setup (one-time):** Create PROJECT_REFERENCE.md when first activating this opinion.

**Hooks:**
- **PLANNING:** READ the reference to understand current project state
- **POST-PLAN (after all chapters in a plan complete):** WRITE — update to reflect new capabilities, decisions, constraints. NOT after every chapter — only after a plan completes.

**PROJECT_REFERENCE.md vs .plans/LOG.md:**
- **PROJECT_REFERENCE.md** = current snapshot. What IS the project right now. Only latest decisions (superseded ones removed). No history.
- **.plans/LOG.md** = execution history. What happened, when, all decisions including superseded. Append-only.
- Both are needed. An agent reads the reference to understand current state. An agent reads the log to understand how we got here.

---

A project reference records what the code cannot tell you: why decisions were made, what was rejected, which constraints are non-obvious, and what the current feature landscape looks like. It is the single most reliable mechanism for maintaining continuity across AI agent sessions, compaction events, and team handoffs.
...
### 8. Update after plan completion, not after every task

The reference is updated **after all chapters in a plan complete** — not after every chapter or task. This prevents churn and keeps the reference stable.

Trigger: `PLAN_TEMPLATE_MASTER.md` Post-Plan Completion section reminds to update.

What to update:
- New features added → update feature map
- Features removed or changed → update status
- Decisions made → add to decision log (remove superseded decisions)
- External services added/changed → update services table
- New constraints discovered → add to "Do Not Break"
- Schema changes → update data model overview
