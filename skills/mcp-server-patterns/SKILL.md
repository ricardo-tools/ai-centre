...
- Delegate to use cases and keep tool handlers as thin controllers. Not: business logic in tool handlers
- Write precise, multi-sentence descriptions that tell the model when and how to use each tool. Not: vague or missing tool descriptions
- Add `.describe()` to every Zod parameter with clear semantics. Not: parameters without descriptions
- Split into focused servers by domain, keeping each under 15 tools. Not: more than 15 tools on a single server
- Return structured JSON from tools and let the model format for the user. Not: tools that return prose
- Keep tool definitions transport-agnostic and wire transport in the entrypoint only. Not: transport-specific code in tool definitions
- Verify permissions in the handler before executing any destructive operation. Not: destructive tools without auth checks
- Extract handler logic into standalone testable functions. Not: tool logic that can only be tested through the MCP server
- Keep resources read-only with no side effects; use tools for state changes. Not: resources that mutate state
- Return structured error messages via `toolError()`. Not: raw error stack traces returned to the model
