...
- Extract business logic to use cases; keep handlers as thin controllers. Not: business logic in handlers
- Return domain objects from repositories via mappers. Not: raw database rows returned from repositories
- Specify needed columns explicitly in queries. Not: selecting all columns by default
- Batch-fetch or use relation loading for related data. Not: database queries inside loops (N+1)
- Catch, log, and return typed errors to the client. Not: exposing raw database errors or stack traces
- Wrap external service calls with retry logic and timeouts, defining critical vs non-critical. Not: external calls without retry or timeout
- Write audit log entries within the same transaction as the state change. Not: audit log writes outside transactions
- Return `Result<T, E>` with `Error` subclasses for expected failures. Not: throwing domain errors for control flow
- Use framework-native mutations for internal operations; reserve API routes for external access. Not: API routes for internal mutations
- Define a staleness policy for every cache entry. Not: caching without an invalidation strategy
