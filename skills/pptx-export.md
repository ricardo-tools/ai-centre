...
- ❌ `#` prefix in hex colour strings → PptxGenJS uses raw hex (`'FF6B00'` not `'#FF6B00'`)
- ❌ Opacity in hex strings → use separate `transparency` property
- ❌ `fill` property for text colour → use `color` property
- ❌ Elements outside safe zone margins → all content within safe margins (see **pptx-export-reference**)
- ❌ Text without overflow handling → always calculate max characters per line and truncate
- ❌ Skipping `pptx.writeFile()` test → every generation must produce a valid, openable file
