...
- Give each slide its own CSS namespace (`.s0-wrap`, `.s3-bars`). Not: generic shared layout classes (`.content-slide`)
- Set light theme as the default (`data-theme="light"`). Not: dark theme as default
- Keep the file standalone with zero external dependencies beyond Google Fonts and PptxGenJS CDN. Not: external CSS/JS dependencies
- Implement full keyboard navigation (arrow/space/home/end/escape/F). Not: missing keyboard navigation
- Implement swipe left/right touch navigation. Not: missing touch navigation
- Include a light/night theme toggle in the footer. Not: missing theme toggle
- Pass the pptx-export checklist before delivering any PPTX export. Not: PPTX export without validation
- Use `var(--color-*)` semantic tokens or theme-derived values for all CSS. Not: hardcoded colours
- Persist slide position via URL hash (`#slide-N`) across reloads. Not: missing URL hash persistence
