...
- ❌ Storing blob URLs only in memory or local state → always persist URLs in the database
- ❌ Uploading without size validation → always check file size before uploading
- ❌ Using `addRandomSuffix: false` without guaranteeing unique filenames → collisions overwrite existing files silently
- ❌ Assuming private access → all Vercel Blob URLs are public; use an access-control proxy for sensitive documents
- ❌ Deleting database records without deleting the blob → orphaned blobs waste storage and incur costs
- ❌ Committing `.blob-storage/` to source control → add it to `.gitignore`
- ❌ Hardcoding the blob store URL in application code → always use the URL returned by `put()`
