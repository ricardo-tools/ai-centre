...
- Use the anon key for browser clients; the service role key bypasses RLS. Not: using the service role key in browser code
- Enable RLS on every table with user data. Not: disabling RLS on tables with user data
- Use `USING` to filter reads and `WITH CHECK` to validate writes in RLS policies. Not: confusing `USING` vs `WITH CHECK`
- Call `removeChannel` in the cleanup function when subscribing to realtime. Not: subscribing without cleaning up the channel on unmount
- Keep `SUPABASE_SERVICE_ROLE_KEY` server-only. Not: storing it in `NEXT_PUBLIC_` variables
- Use the admin client only in server-side code. Not: using `supabaseAdmin` from Client Components
- Generate types from the database schema to keep queries typed and safe. Not: skipping type generation
