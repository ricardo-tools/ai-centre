...
- Hash passwords with bcrypt, scrypt, or Argon2. Not: storing passwords in plain text
- Store tokens in httpOnly cookies or in-memory. Not: using localStorage for tokens
- Use short-lived access tokens with refresh token rotation. Not: long-lived JWTs with no refresh mechanism
- Return generic messages like "If that email exists..." for auth errors. Not: revealing whether an account exists
- Enforce per-account and per-IP rate limits on login and OTP endpoints. Not: skipping rate limiting
- Use bcrypt, scrypt, or Argon2 for password hashing. Not: MD5 or SHA-1 which are insecure for passwords
- Guard dev auth bypass with a `NODE_ENV` check so it never reaches production. Not: deploying dev bypass to production
- Implement "remember me" with refresh token rotation. Not: indefinite token lifetimes
