...
- Use `@clerk/nextjs` client-side hooks in Client Components. Not: importing `@clerk/nextjs/server` which crashes the browser bundle
- Protect pages via middleware. Not: checking auth client-side, which flashes unauthorized content
- Keep the Clerk secret key server-only. Not: storing it with a `NEXT_PUBLIC_` prefix that exposes it to the browser
- Verify webhook signatures with `svix` on every webhook request. Not: skipping signature verification
- Maintain a local users table synced via webhooks for foreign keys, joins, and offline access. Not: relying solely on Clerk for user data
- Use `useAuth()` in Client Components for auth access. Not: calling `auth()` which is server-only
- Define role constants once in a central location. Not: hardcoding role strings in multiple places
