...
- ❌ Hardcoding model names without checking if they're still available → use `search` and `find` to discover current models
- ❌ Generating with an expensive model before the prompt is refined → iterate with the cheapest model first
- ❌ Generating without calling `find` first → parameter schemas vary between models; check before calling
- ❌ Skipping `estimate_cost` for video or high-fidelity models → always estimate before expensive generations
- ❌ Delivering AI-generated media without reviewing for artifacts → check hands, faces, text, temporal consistency
- ❌ Vague one-line prompts ("a cool image") → use the four-part prompt structure
- ❌ Changing both the prompt and the seed at the same time → isolate variables to understand what's improving the output
- ❌ Using AI generation for brand assets that need pixel-perfect precision → use design tools instead
