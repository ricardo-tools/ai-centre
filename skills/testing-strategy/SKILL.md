...
- Use Vitest for new projects (same API as Jest, 5-10x faster, native TypeScript). Not: Jest in new projects
- Use Playwright for E2E testing (faster, cheaper CI, native parallelism, better browser coverage). Not: Cypress
- Test behaviour directly with assertions. Not: snapshot testing (false confidence, rubber-stamped updates)
- Let internal collaborators (domain objects, utilities) run for real; mock only external boundaries. Not: mocking internal collaborators
- Test inputs and outputs. Not: testing implementation details (function call counts, internal state)
- Test behaviours that matter, grouping by behaviour not by file. Not: one-to-one test-to-file mapping
- Only write tests that earn their keep; skip if TypeScript or E2E already covers the path. Not: tests without a clear reason to exist
- Use Fishery factories for consistent, realistic test data. Not: test data as inline literals
- Use MSW consistently across unit, component, and E2E levels. Not: different mock strategies per test level
- Test async Server Components with E2E (Playwright) since they cannot be unit tested. Not: skipping E2E for async Server Components
