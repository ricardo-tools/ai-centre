...
Test what the system does (given this input, expect this output), not how it does it internally. Tests that check implementation details break when you refactor — the opposite of their purpose. For the testing distribution, level guidance, and practical test patterns, see **flow-tdd**.
...
For the testing trophy distribution, level guidance, and practical test patterns, see **flow-tdd**. The key quality insight: static analysis (types, linting) changes the process so certain defects cannot form, while integration and E2E tests catch what prevention misses.
...
TDD's real purpose is giving developers confidence to change code — without tests, quality inevitably degrades over time. The test is not the product; the confidence to change and improve is. For testing mechanics and patterns, see **flow-tdd**.
